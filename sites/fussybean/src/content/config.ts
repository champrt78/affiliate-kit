...
/* FussyBean content schema — upgraded 2026-05-28 to match the current
   mywildlifecam schema (bottomLine / scorecard / buyIf / flaws / faq / bgTheme),
   PLUS a `pillar` FK field (→ navigation.pillars[].slug in site-config.json).
   All new fields are optional so DRAFT pieces render with graceful fallback.
   Empty `bottomLine.verdict` is the DRAFT/noindex gate (see page templates). */
...
/* bgTheme — per-page background theme. FussyBean ships "solid" only for now
   (espresso band, no photo gutters). The enum mirrors MWC for schema parity;
   coffee-scene gutter photos are a post-launch iteration. Defaults to "solid". */
