...
## Steal-from-Aaron review (2026-06-12) — file, work later

Full analysis + reasoning: `docs/brainstorms/2026-06-12-steal-from-aaron-master-prompts.md`. Reviewed 5 of Aaron's master-prompt docs. Reframe: his kit is a volume+autonomy machine with **no quality gate** — the opposite of the Bottom Line gate, which is our moat. So these steals **harden the gate**, they don't adopt his no-gate engine. None reorder the locked Magic Go sequence.

- [ ] **Off-page backlink playbook (highest-value SEO steal).** We're thin on external backlinks (flagged in `Session_2026-06-12.md`). Aaron's `STANDALONE-OFFPAGE-SEO-PROMPT` is a concrete manual outreach checklist: HARO, guest posts, broken-link building, resource-page links, a data study, Reddit/Quora. **Steal as a tactics checklist, not code** — it's outreach labor. Pairs with the MWC→satellite internal-link lever already in flight.
- [ ] **C1 — Post-deploy visual-regression screenshot pass (Playwright MCP, already available).** *Additive* to existing lints, NOT Aaron's "screenshot is the only proof" absolutism (his is calibrated to a fragile React SPA; our static Astro lacks most of those failure classes). Thin smoke check on one money page/site to catch the narrow class greps miss: a marketing *composite* image rendering as a sliver in our 1:1 cards, or a mobile CTA/image overlap. Scope as a smoke check, not a gate.
- [ ] **C2 — ASIN-to-name alignment check.** Verify the ASIN resolves to a product whose name matches our page (a wrong-but-live ASIN sends buyers to a competitor and still returns 200). Distinct from the planned liveness guard (`magic-go-v1-plan.md` ~line 424) — add alongside it: on scaffold, fetch ASIN title (Canopy/PA-API), fuzzy-match against the piece's product name, flag mismatches to the failures queue.
- [ ] **C3 — Amazon tagged-search-link fallback.** For products with no verified ASIN, link to `/s?k=Product+Name&tag=...` (never-404s) instead of a guessed `/dp/{ASIN}` that shows the wrong product. Pairs with Item 1: if no verified ASIN AND no other network has it, degrade to tagged search.
- [ ] **PAA-into-FAQ (minor).** Pipe People-Also-Ask questions into the FAQ sections our pieces already have — better AI-Overview citation surface, truthful under doctrine. Low priority.
- **Explicitly NOT stealing** (reasons in the brainstorm): Rule 4 "no human checkpoints" (antithesis of the gate), Prerender.io (SPA crutch; static Astro already emits full HTML), GA4 + Consent Mode v2 (we're cookieless via Cloudflare Web Analytics — adding GA4 forces a cookie banner = regression), niche-scoring matrix, the 500–1k volume play. Reference-only deploy note: Wrangler **v3.20** + token perm **User→Memberships→Read** is the fix if our deploy ever throws a `/memberships` auth error.
